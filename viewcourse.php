...
<html>
  <head>
    <title>View Workout | Endurify</title>
    <meta charset="UTF-8">
          ...
          <div class="main-content">
            <div class="dash-item">
              ...
              <?php
                ...
                $query = "SELECT c.*, ec.name as category_name FROM courses c JOIN exercise_categories ec ON c.category_id = ec.category_id WHERE course_id = ?";
                ...
                if ($row = mysqli_fetch_assoc($result)) {
                  echo "
                    <h1 class='workout-title'>{$row['course_name']}</h1>
                ...
                $course_id = mysqli_real_escape_string($dbc, $_GET['course']);
                $course_exercises_sql = "SELECT ce.*, e.*, ec.name AS category_name FROM course_exercises ce JOIN exercises e ON ce.exercise_id = e.exercise_id JOIN exercise_categories ec ON e.category = ec.category_id WHERE ce.course_id = ? ORDER BY sequence";

                if ($stmt = mysqli_prepare($dbc, $course_exercises_sql)) {
                    mysqli_stmt_bind_param($stmt, 'i', $id);
                    mysqli_stmt_execute($stmt);
                    $result = mysqli_stmt_get_result($stmt);
